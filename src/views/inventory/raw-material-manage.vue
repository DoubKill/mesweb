<template>
  <div v-loading="loading" class="app-container outbound_manage">
    ...
    <el-button v-permission="['material_outbound_plan','norman']" class="button-right" @click="normalOutbound">正常出库</el-button>
    <el-button v-permission="['material_outbound_plan','assign']" class="button-right" @click="assignOutbound">指定出库</el-button>
    ...
    >
      ...
      <el-table-column label="操作" align="center" width="220">
        <template v-if="scope.row.status === 4" slot-scope="scope">
          <el-button-group>
            <el-button v-permission="['material_outbound_plan','manual']" size="mini" type="primary" @click="manualDelivery(scope.row)">人工出库</el-button>
            <el-button v-permission="['material_outbound_plan','change']" size="mini" type="warning" @click="demandQuantity(scope.$index,scope.row)">编辑</el-button>
            <el-button v-permission="['material_outbound_plan','close']" size="mini" type="info" @click="closePlan(scope.$index,scope.row)">关闭</el-button>
          </el-button-group>
        </template>
      </el-table-column>
...
import { materialPlanManagement } from '@/api/base_w'
  ...
  methods: {
    async getList() {
      try {
        this.loading = true
        this.tableData = []
        const data = await materialPlanManagement('get', null, { params: this.search })
    ...
    async visibleMethodSubmit(val) {
      try {
        await materialPlanManagement('post', null, { data: [val] })
    ...
    async submitDemandQuantity() {
      try {
        ...
        await materialPlanManagement('put', row.id, { data: obj })
