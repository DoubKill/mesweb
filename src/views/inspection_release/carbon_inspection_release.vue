...
import { thStorageSummary, thStorage, thRelease, wmsExceptHandle } from '@/api/jqy'
export default {
  ...
  data() {
    return {
      ...
      loading: false,
      rules: {
        quality_status: [{ required: true, message: '不能为空', trigger: 'change' }],
    ...
    async generateFunAbnormal() {
      ...
      try {
        ...
        if (this.abnormalForm.result === '放行') {
          await thRelease('post', null, { data: { status: this.abnormalForm.quality_status, tracking_nums: this.trackingList, operation_type: this.abnormalForm.result }})
        }
