    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button v-permission="['raw_level','delete']" size="small" @click="clickDelete(scope.$index,scope.row)">删除</el-button>
        </template>
      </el-table-column>
    </el-table>
    <page
...
export default {
  ...
  methods: {
    ...
    async getList() {
      try {
        this.loading = true
        const data = await levelResultRaw('get', null, { params: { page: this.page }})
    ...
    clickDelete(index, row) {
      this.$confirm(
        ...
        await levelResultRaw('delete', row.id)
      ...
      this.$refs.ruleForm.validate(async(valid) => {
        if (valid) {
          try {
            ...
            await levelResultRaw('post', null, { data: this.ruleForm })
