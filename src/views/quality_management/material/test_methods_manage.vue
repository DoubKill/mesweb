  ...
  <div class="app-container">
    ...
    <el-form :inline="true">
      ...
      <el-form-item style="float: right">
        <el-button v-permission="['raw_test_method', 'add']" @click="handleCreate">
          创建试验方法
        </el-button>
      </el-form-item>
    </el-form>
    ...
    >
      ...
      <el-table-column label="操作" align="center">
        <template slot-scope="{row}">
          ...
          <el-button v-permission="['raw_test_method', 'change']" size="mini" @click="handleUpdate(row)">编辑</el-button>
        </template>
      </el-table-column>
    </el-table>
...
import { testTypesRaw, testMethodsRaw } from '@/api/base_w_three'
import DetectionIndex from '../components/qualityDetectionIndex'
...
export default {
  ...
  methods: {
    ...
    async getTestMethodList() {
      ...
      try {
        const response = await testMethodsRaw('get', null, { params: this.getParams })
    ...
    createData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          testMethodsRaw('post', null, { data: this.formData }).then(() => {
    ...
    updateData() {
      this.$refs['dataForm'].validate((valid) => {
        if (valid) {
          ...
          testMethodsRaw('put', formData.id, { data: formData }).then(() => {
            this.dialogFormVisible = false
    ...
    handleDelete(row, index) {
      this.$confirm(`确定删除${row.name}?`, '提示', {
        ...
        testMethodsRaw('delete', row.id).then(response => {
          this.$message({
            type: 'success',
            message: '操作成功!'
          })
