    ...
    <el-row>
      <el-col :span="12">
        <el-form :inline="true">
          <el-form-item style="float: right">
            <el-button
              v-permission="['raw_test_type', 'add']"
        ...
        >
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button-group>
                <el-button
                  v-permission="['raw_test_type', 'change']"
                  size="mini"
      ...
      <el-col :span="12">
        <el-form :inline="true">
          <el-form-item style="float: right">
            <el-button
              v-permission="['raw_test_type', 'pointAdd']"
        ...
        >
          ...
          <el-table-column label="操作">
            <template slot-scope="scope">
              <el-button-group>
                <el-button
                  v-permission="['raw_test_type', 'pointChange']"
                  size="mini"
...
<script>
import { testTypesRaw, testIndicatorsRaw, dataPointsRaw } from '@/api/base_w_three'
...
export default {
  ...
  methods: {
    ...
    getTestTypesList() {
      testTypesRaw('get', null, { params: this.getParams }).then(response => {
    ...
    handleCreateTestType: function() {
      this.$refs.createTestTypeForm.validate((valid) => {
        if (valid) {
          ...
          testTypesRaw('post', '', { data: this.testTypeForm })
            .then(response => {
    ...
    handleEditTestType: function() {
      this.$refs.editTestTypeForm.validate((valid) => {
        if (valid) {
          testTypesRaw('put', this.testTypeForm.id, { data: this.testTypeForm })
            .then(response => {
    ...
    handleTestTypesCurrentRowChange: function(row) {
      ...
      dataPointsRaw('get', null, { params: { all: 1, test_type_id: row.id }})
        .then(response => {
    ...
    handleCreateDataPoints: function() {
      this.$refs.createDataPointsForm.validate((valid) => {
        if (valid) {
          dataPointsRaw('post', null, { data: this.dataPointsForm })
            .then(response => {
    ...
    handleEditDataPoints: function() {
      this.$refs.editDataPointsForm.validate((valid) => {
        if (valid) {
          dataPointsRaw('put', this.dataPointsForm.id, { data: this.dataPointsForm })
            .then(response => {
