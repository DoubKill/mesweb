  ...
  <div>
    ...
    <el-form :inline="true">
      <el-form-item style="float: right">
        <el-button
          v-permission="['raw_test_indicator', 'add']"
    ...
    >
      ...
      <el-table-column label="操作">
        <template slot-scope="scope">
          <el-button-group>
            <el-button
              v-permission="['raw_test_indicator', 'change']"
              size="mini"
...
export default {
  ...
  methods: {
    getTestIndicatorsList() {
      testIndicatorsRaw('get', null, { params: this.getParams }).then(response => {
    ...
    handleCreateTestIndicators: function() {
      this.$refs.createTestIndicatorsForm.validate((valid) => {
        if (valid) {
          testIndicatorsRaw('post', null, { data: this.TestIndicatorsForm })
            .then(response => {
    ...
    handleEditTestIndicators: function() {
      this.$refs.editTestIndicatorsForm.validate((valid) => {
        if (valid) {
          testIndicatorsRaw('put', this.TestIndicatorsForm.id, { data: this.TestIndicatorsForm })
            .then(response => {
