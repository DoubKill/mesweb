      ...
      <div v-loading="loading">
        <el-form :inline="true">
          ...
          <el-form-item label="" style="float:right">
            <el-button :disabled="disabledSubmit" type="primary" @click="submitClick">保存</el-button>
          </el-form-item>
        </el-form>
...
<script>
import { matDataPointIndicators, matDataPointIndicatorsHistory, batchSetMethods } from '@/api/base_w'
...
export default {
  ...
  methods: {
    ...
    async submitClick() {
      try {
        this.objEdit.tableData.forEach(d => {
          if (!d.lower_limit || !d.upper_limit || !d.level) {
            throw new Error('每行数据必填')
          }
        })
        this.disabledSubmit = true
        const obj = {
          indicator_data: this.objEdit.tableData,
          test_method_ids: this.objEdit.test_method_ids
        }
        await batchSetMethods('post', null, { data: obj })
