...
<script>
import { matDataPointIndicatorsRaw } from '@/api/base_w_three'
import qualityGradeManageSelect from '../components/qualityGradeManageSelect'
import { deepClone } from '@/utils/index'

export default {
  components: { qualityGradeManageSelect },
  props: {
    editShow: {
      type: Boolean,
      default: false
  ...
  methods: {
    ...
    async getInfo() {
      try {
        const data = await matDataPointIndicatorsRaw('get', null, { params: {
    ...
    async submitClick(row) {
      try {
        ...
        await matDataPointIndicatorsRaw(_api, id, { data: row })
    ...
    async clickDelete(index, row) {
      this.$confirm(
        ...
        if (row.id) {
          await matDataPointIndicatorsRaw('delete', row.id)
        }
