...
export default {
  ...
  methods: {
    async getList() {
      try {
        if (this.$route.fullPath !== '/rubber' && this.storeName === '混炼胶库' && (!this.station || !this.status)) {
          this.$message.info('请选择出库口 和 品质状态')
          return
        }
        this.loading = true
        const _api = this.storeName === '混炼胶库' && this.$route.fullPath !== '/rubber' ? bzMixinInventorySummary : materialCount
